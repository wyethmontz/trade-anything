"""Walk-forward backtest of the src.advisor rule against historical price data for
the active asset (see src/asset_config.py).
...
def run_backtest(
    ...
    data_symbol = symbol or get_active_asset().data_symbol
    raw_df = get_price_data(data_symbol, period=period, interval=interval)
    if raw_df.empty:
...
def report(
    trades: list[Trade], account_balance: float, risk_pct: float, spread_usd: float, unit_label: str = "unit"
) -> None:
    if not trades:
    ...
    print(f"Trades: {len(trades)}  (spread modeled at ${spread_usd:.2f}/{unit_label} round trip)")
    print("=" * 62)
...
if __name__ == "__main__":
    ...
    account_balance = asset.default_account_balance
    risk_pct = 0.5
    spread_usd = asset.spread_estimate
    trades = run_backtest(account_balance=account_balance, risk_pct=risk_pct, spread_usd=spread_usd)
    report(trades, account_balance=account_balance, risk_pct=risk_pct, spread_usd=spread_usd, unit_label=asset.unit_label)
